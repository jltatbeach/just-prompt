...
def parse_thinking_suffix(model: str) -> Tuple[str, int]:
    ...
    try:
        ...
        if thinking_budget < 1024:
            logger.warning(
                f"Thinking budget {thinking_budget} below minimum (1024), using 1024 instead"
            )
            thinking_budget = 1024
        elif thinking_budget > 16000:
            logger.warning(
                f"Thinking budget {thinking_budget} above maximum (16000), using 16000 instead"
            )
...
def prompt_with_thinking(text: str, model: str, thinking_budget: int) -> str:
    ...
    try:
        ...
        logger.info(
            f"Sending prompt to Anthropic model {model} with thinking budget {thinking_budget}"
        )
        ...
        raise ValueError(
            f"Failed to get response from Anthropic with thinking: {str(e)}"
        )
