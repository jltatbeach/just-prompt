...
def validate_models_prefixed_by_provider(
    ...
    for model_string in models_prefixed_by_provider:
        try:
            ...
            logger.error(
                f"Validation error for model string '{model_string}': {str(e)}"
            )
            raise
...
def print_provider_availability(detailed: bool = True) -> None:
    ...
    if detailed and unavailable:
        ...
        logger.warning(
            "The following providers are unavailable due to missing API keys:"
        )
        for provider in unavailable:
            env_var = env_vars.get(provider)
            if env_var:
                logger.warning(
                    f"  - {provider}: Missing environment variable {env_var}"
                )
            else:
