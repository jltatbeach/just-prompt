...
def test_prompt():
    ...
    response = anthropic.prompt(
        "What is the capital of France?", "claude-3-5-haiku-20241022"
    )
...
def test_parse_thinking_suffix():
    ...
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet") == (
        "claude-3-7-sonnet",
        0,
    )
    assert anthropic.parse_thinking_suffix("claude-3-5-haiku-20241022") == (
        "claude-3-5-haiku-20241022",
        0,
    )

    # Test cases with supported model and k suffixes
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:1k") == (
        "claude-3-7-sonnet-20250219",
        1024,
    )
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:4k") == (
        "claude-3-7-sonnet-20250219",
        4096,
    )
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:15k") == (
        "claude-3-7-sonnet-20250219",
        15360,
    )  # 15*1024=15360 < 16000

    # Test cases with supported model and numeric suffixes
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:1024") == (
        "claude-3-7-sonnet-20250219",
        1024,
    )
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:4096") == (
        "claude-3-7-sonnet-20250219",
        4096,
    )
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:8000") == (
        "claude-3-7-sonnet-20250219",
        8000,
    )

    # Test cases with non-supported model
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet:1k") == (
        "claude-3-7-sonnet",
        0,
    )
    assert anthropic.parse_thinking_suffix("claude-3-5-haiku:4k") == (
        "claude-3-5-haiku",
        0,
    )

    # Test cases with out-of-range values (should adjust to valid range)
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:500") == (
        "claude-3-7-sonnet-20250219",
        1024,
    )  # Below min 1024, should use 1024
    assert anthropic.parse_thinking_suffix("claude-3-7-sonnet-20250219:20000") == (
        "claude-3-7-sonnet-20250219",
        16000,
    )  # Above max 16000, should use 16000
...
def test_prompt_with_thinking():
    ...
    response = anthropic.prompt(
        "What is the capital of Spain?", "claude-3-7-sonnet-20250219:1k"
    )
    ...
    response = anthropic.prompt(
        "What is the capital of Germany?", "claude-3-7-sonnet-20250219:2k"
    )
    ...
    response = anthropic.prompt(
        "What is the capital of Italy?", "claude-3-7-sonnet-20250219:500"
    )
