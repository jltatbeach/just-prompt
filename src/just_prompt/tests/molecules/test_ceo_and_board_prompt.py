...
class TestCEOAndBoardPrompt:
    ...
    def test_ceo_and_board_prompt_success(
        self,
        mock_file,
        mock_prompt,
        mock_prompt_from_file_to_file,
        mock_environment,
        tmpdir,
    ):
        ...
        mock_prompt_from_file_to_file.assert_called_once_with(
            input_file, ["a:claude-3", "o:gpt-4o"], str(tmpdir)
        )
        ...
        assert (
            "<original-question>Test prompt question</original-question>" in prompt_arg
        )
        ...
        )  # Should be called at least twice - once for prompt XML and once for decision
        ...
        for call_args in mock_file.call_args_list:
            ...
            if (
                str(args[0]) == expected_prompt_file
                and args[1] == "w"
        ...
        for call_args in mock_file.call_args_list:
            args, kwargs = call_args
            if (
                str(args[0]) == expected_output_file
                and args[1] == "w"
                and kwargs.get("encoding") == "utf-8"
            ):
    ...
    @patch("just_prompt.molecules.ceo_and_board_prompt.prompt")
    @patch("builtins.open", new_callable=mock_open, read_data="Test prompt question")
    def test_ceo_and_board_prompt_with_defaults(
        self,
        mock_file,
        mock_prompt,
        mock_prompt_from_file_to_file,
        ...
        # Test with defaults
        input_file = "test_prompt.txt"
        result = ceo_and_board_prompt(from_file=input_file, output_dir=str(tmpdir))

        # Assertions
        mock_prompt_from_file_to_file.assert_called_once_with(
            input_file, None, str(tmpdir)
        )
    ...
    @patch("just_prompt.molecules.ceo_and_board_prompt.prompt_from_file_to_file")
    @patch("just_prompt.molecules.ceo_and_board_prompt.prompt")
    def test_ceo_and_board_prompt_file_not_found(
        ...
        mock_open_instance.side_effect = FileNotFoundError(
